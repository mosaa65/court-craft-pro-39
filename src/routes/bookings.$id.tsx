...
export const Route = createFileRoute("/bookings/$id")({
  ...
  errorComponent: ({ error }) => (
    <AppShell>
      <div className="px-6 pt-12 text-center">
        ...
        <Link
          to="/bookings"
          className="mt-6 inline-flex h-11 items-center gap-2 rounded-full bg-ink px-5 text-sm font-bold text-white"
        >
          العودة للحجوزات
        </Link>
      </div>
  ...
  notFoundComponent: () => (
    <AppShell>
      <div className="px-6 pt-12 text-center text-sm text-muted-foreground">
        الحجز غير موجود.
      </div>
    </AppShell>
  ),
      ...
      <main className="space-y-4 px-5 pt-5">
        ...
        <section className="card-elev p-5">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            <MiniStat icon={<Clock className="size-4" />} label="الوقت" value={`${booking.start} — ${booking.end}`} />
            <MiniStat icon={<Clock className="size-4" />} label="المدة" value={formatDuration(durMin)} />
            <MiniStat
              icon={<Coins className="size-4" />}
              label="السعر"
              value={`${toArabicDigits(booking.price)} ر.س`}
            />
          </div>
        </section>
        ...
        {/* Court */}
        {court && (
          <section className="card-elev flex items-center gap-3 p-5">
